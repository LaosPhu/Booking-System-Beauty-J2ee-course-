...
public class BookingRestController {
    ...
    @PostMapping("/api/booking/update-status")
    public ResponseEntity<?> updateBookingStatus() {
        try {
            // 1. Get all bookings
            List<Booking> bookings = bookingRepository.findAll(); //  method in BookingService
            if (bookings == null || bookings.isEmpty()) {
                return ResponseEntity.ok(Map.of("message", "No bookings found to update."));
            }

            // 2. Iterate through bookings and check conditions
            for (Booking booking : bookings) {
                if (booking.getPaymentMethod().equalsIgnoreCase("Cash")) {
                    ...
                    if (now.isAfter(appointmentTime) && booking.getBookingStatus().equalsIgnoreCase("Pending")) {
                        ...
                        bookingRepository.save(booking); //  method in BookingService
                    }
                }
            }
            return ResponseEntity.ok(Map.of("message", "Booking statuses updated successfully."));

        } catch (Exception e) {
            // 4. Handle errors
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("message", "Error updating booking statuses: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> updateBookingStatus(@PathVariable long bookingId) {
        ...
        if (booking.get().getPaymentMethod().equalsIgnoreCase("Cash")) {
            LocalDateTime appointmentTime = booking.get().getAppointmentDateTime();
            LocalDateTime now = LocalDateTime.now();

            if (now.isAfter(appointmentTime) && booking.get().getBookingStatus().equalsIgnoreCase("Pending")) {
                // 3. Update booking status to "Completed"
                booking.get().setBookingStatus("Completed");
                bookingRepository.save(booking.get()); //  method in BookingService
            }
